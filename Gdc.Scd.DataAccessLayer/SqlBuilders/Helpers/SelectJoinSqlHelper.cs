...
namespace Gdc.Scd.DataAccessLayer.SqlBuilders.Helpers
{
    public class SelectJoinSqlHelper : SelectWhereSqlHelper, IJoinSqlHelper<SelectJoinSqlHelper>        
    {
        ...
        public SelectJoinSqlHelper Join(BaseEntityMeta meta, string referenceFieldName, string joinedTableAlias = null, string metaTableAlias = null, JoinType joinType = JoinType.Inner)
        {
            return new SelectJoinSqlHelper(this.joinSqlHelper.Join(meta, referenceFieldName, joinedTableAlias, metaTableAlias, joinType));
        }
        ...
        public SelectJoinSqlHelper JoinQuery(ISqlBuilder query, ISqlBuilder condition, string alias, JoinType type = JoinType.Inner)
        {
            return new SelectJoinSqlHelper(
                this.joinSqlHelper.JoinQuery(query, condition, alias, type));
        }

        public SelectJoinSqlHelper JoinQuery(SqlHelper query, ConditionHelper condition, string alias, JoinType type = JoinType.Inner)
        {
            return new SelectJoinSqlHelper(
                 this.joinSqlHelper.JoinQuery(query, condition, alias, type));
        }
