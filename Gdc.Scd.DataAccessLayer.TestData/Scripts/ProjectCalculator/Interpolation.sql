...
IF OBJECT_ID('[ProjectCalculator].[GetReinsuranceFlatfee]') IS NOT NULL
    DROP FUNCTION [ProjectCalculator].[GetReinsuranceFlatfee]
GO
...
IF OBJECT_ID('[ProjectCalculator].[GetReinsuranceUpliftFactor]') IS NOT NULL
    DROP FUNCTION [ProjectCalculator].[GetReinsuranceUpliftFactor]
GO
...
SELECT
	[Reinsurance].*,
	[CalculatingProjectItems].[Id] AS ProjectItemId,
	[CalculatingProjectItems].[Duration_Months] AS ProjectItemDurationMonths,
	[CalculatingProjectItems].[ReactionTime_Minutes] AS ProjectItemReactionTimeMinutes,
	[CalculatingProjectItems].[Availability_Start_Day] AS ProjectItemAvailabilityStartDay,
	[CalculatingProjectItems].[Availability_Start_Hour] AS ProjectItemAvailabilityStartHour,
	[CalculatingProjectItems].[Availability_End_Day] AS ProjectItemAvailabilityEndDay,
	[CalculatingProjectItems].[Availability_End_Hour] AS ProjectItemAvailabilityEndHour
FROM
...
SELECT
	t.[ProjectItemId],
	t.[ProjectItemDurationMonths],
	t.[ReinsuranceFlatfee],
	[Duration].[Name] AS DurationName,
	[Duration].[Value] * 12 AS DurationMonths
FROM
(
	SELECT
		[ProjectItemId],
		[ProjectItemDurationMonths],
		[Duration],
...
CREATE FUNCTION [ProjectCalculator].[GetReinsuranceFlatfee](@projectItemIds [dbo].[ListID] READONLY)
RETURNS TABLE
AS
RETURN
	WITH [ReinsuranceDuration] AS 
	(
		SELECT
			*
		FROM
			[ProjectCalculator].[GetReinsuranceDuration](@projectItemIds) AS t
		WHERE
			t.[ProjectItemDurationMonths] <= t.[DurationMonths] AND 
			t.[ReinsuranceFlatfee] IS NOT NULL
	),
	[ReinsuranceDurationWihtRowNumber] AS
	(
		SELECT
			[ReinsuranceDuration].*,
			ROW_NUMBER() OVER(PARTITION BY [ReinsuranceDuration].[ProjectItemId] ORDER BY [ReinsuranceDuration].[DurationMonths]) AS RowNumber
		FROM
			[ReinsuranceDuration]
	)
	SELECT
		*
	FROM
		[ReinsuranceDurationWihtRowNumber]
	WHERE
		[RowNumber] = 1
GO
...
SELECT
	t.[ProjectItemId],
	t.[ProjectItemReactionTimeMinutes],
	t.[ProjectItemAvailabilityStartDay],
	t.[ProjectItemAvailabilityStartHour],
	t.[ProjectItemAvailabilityEndDay],
	t.[ProjectItemAvailabilityEndHour],
	t.[ReinsuranceUpliftFactor],
	[ReactionTimeAvailability].[Name] AS ReactionTimeAvailabilityName,
	[ReactionTime].[Minutes] AS ReactionTimeMinutes,
	[Availability].[Name] AS [Availability],
	[Availability].[Start_Day] AS AvailabilityStartDay,
    [Availability].[Start_Hour] AS AvailabilityStartHour,
    [Availability].[End_Day] AS AvailabilityEndDay,
    [Availability].[End_Hour] AS AvailabilityEndHour
FROM
(
	SELECT
		[ProjectItemId],
		[ProjectItemReactionTimeMinutes],
		[ProjectItemAvailabilityStartDay],
		[ProjectItemAvailabilityStartHour],
		[ProjectItemAvailabilityEndDay],
		[ProjectItemAvailabilityEndHour],
...
CREATE FUNCTION [ProjectCalculator].[GetReinsuranceUpliftFactor](@projectItemIds [dbo].[ListID] READONLY)
RETURNS TABLE
AS
RETURN
	WITH [ReinsuranceReactionTimeAvailability] AS 
	(
		SELECT
			*
		FROM
			[ProjectCalculator].[GetReinsuranceReactionTimeAvailability](@projectItemIds) AS t
		WHERE
			t.[ReinsuranceUpliftFactor] IS NOT NULL AND
			t.[ProjectItemReactionTimeMinutes] <= t.[ReactionTimeMinutes] AND
			t.[ProjectItemAvailabilityStartDay] >= t.[AvailabilityStartDay] AND
			t.[ProjectItemAvailabilityStartHour] >= t.[AvailabilityStartHour] AND
			t.[ProjectItemAvailabilityEndDay] <= t.[AvailabilityEndDay] AND
			t.[ProjectItemAvailabilityEndHour] <= t.[AvailabilityEndHour]
	),
	[ReinsuranceWithRowNumber] AS 
	(
		SELECT
			[ReinsuranceReactionTimeAvailability].*,
			ROW_NUMBER() OVER
			(
				PARTITION BY 
					[ProjectItemId] 
				ORDER BY 
					[ReactionTimeMinutes] ASC, 
					[AvailabilityStartDay] DESC,
					[AvailabilityStartHour] DESC,
					[AvailabilityEndDay] ASC,
					[AvailabilityEndHour] ASC
			) 
			AS RowNumber
		FROM
			[ReinsuranceReactionTimeAvailability]
...
BEGIN
	...
	SET
		...
		[Reinsurance_Flatfee] = [ReinsuranceFlatfee].[ReinsuranceFlatfee],
		[Reinsurance_UpliftFactor] = [ReinsuranceUpliftFactor].[ReinsuranceUpliftFactor]
	FROM
		#CalculatingProjectItems AS [CalculatingProjectItems]
	LEFT JOIN
	...
	LEFT JOIN
		[ProjectCalculator].[GetReinsuranceFlatfee](@projectItemIds) AS [ReinsuranceFlatfee] ON [ReinsuranceFlatfee].[ProjectItemId] = [CalculatingProjectItems].[Id]
	LEFT JOIN
		[ProjectCalculator].[GetReinsuranceUpliftFactor](@projectItemIds) AS [ReinsuranceUpliftFactor] ON [ReinsuranceUpliftFactor].[ProjectItemId] = [CalculatingProjectItems].[Id]
	WHERE
