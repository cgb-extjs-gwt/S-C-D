            ...
            using (var transaction = this._repositorySet.GetTransaction())
            {
                try
                {
                    ...
                    foreach (var swCombination in swInfo)
                    {
                        ...
                        if (digit == null)
                        {
                            this._logger.Warn(PorImportLoggingMessage.UNKNOW_DIGIT, nameof(SwDigitLicense), swCombination.Software_Lizenz_Digit);
                            continue;
                        }
                        var license = licenses.FirstOrDefault(l => l.Name.Equals(swCombination.Software_Lizenz, StringComparison.OrdinalIgnoreCase));
                        if (license == null)
                        {
                            this._logger.Warn(PorImportLoggingMessage.UNKNOWN_LICENSE, nameof(SwDigitLicense), swCombination.Software_Lizenz);
                            continue;
                        }
                        ...
                        {
                            _logger.Debug(PorImportLoggingMessage.ADDING_SWDIGIT_SWLICENSE, license.Name, digit.Name);
