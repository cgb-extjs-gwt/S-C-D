...
import {
  ...
  APP_LOAD_DATA
} from "../../Layout/Actions/AppActions";
...
import {
  ...
  UsingInfo,
  FieldType
} from "../../Common/States/CostMetaStates";
import {
  filterCostEditorItems,
  findCostBlockByState,
  findCostBlock,
  findApplication,
  findCostElement
} from "../Helpers/CostEditorHelpers";
import {
  COST_BLOCK_INPUT_LOAD_COST_ELEMENT_DATA,
  CostBlockAction,
  CostElementMetaAction,
  InputLevelAction,
...
const buildCostBlockState = (costBlockMeta: CostBlockMeta) =>
  <CostBlockState>{
    costBlockId: costBlockMeta.id,
    selectedRegionId: null,
    costElements: {
      selectedItemId: null,
      list: filterCostEditorItems(costBlockMeta.costElements).map(
        costElementMeta =>
          <CostElementState>{
            costElementId: costElementMeta.id,
            inputLevels: {
              selectedItemId: null,
              list: costElementMeta.inputLevels.map(
                inputLevel =>
                  <InputLevelState>{
...
const initByMeta: Reducer<CostEditorState, LoadingAppDataAction> = (
  state,
  { data: { meta } }
) => {
      ...
      appMetas.length > 0
        ? {
            ...
            list: appMetas.map(appMeta => {
              const appCostBlocks = costBockMetas.filter(costBlockMeta =>
                costBlockMeta.applicationIds.includes(appMeta.id)
              );
    ...
    costElements: {
      ...
      list: costBlock.costElements.list.map(costElement => ({
        ...
        inputLevel: {
          ...
          selectedItemId: clearSelected
            ? null
            : costElement.inputLevels.selectedItemId
        }
      }))
    }
  };
...
const selectApplication: Reducer<CostEditorState, ItemSelectedAction> = (
  state,
  { selectedItemId }
) => {
  return {
...
const buildCostBlockChanger = <TAction extends CostBlockAction, TParam = any>(
  ...
  return mapCostBlock(state, applicationId, costBlockId, costBlock =>
    changeFn(costBlock, costBlockAction, state, addionalParam)
  );
};

const buildCostElementChanger = <TAction extends CostElementAction>(
  costElementFn: (
    costElement: CostElementState,
    action: TAction
  ) => CostElementState
) =>
  buildCostBlockChanger<TAction>((costBlock, action) => ({
    ...costBlock,
    costElements: {
      ...costBlock.costElements,
      list: mapIf(
...
const buildInputLevelFilterChanger = <TAction extends InputLevelAction>(
  inputLevelFn: (
    costElement: InputLevelState,
    action: TAction
  ) => InputLevelState
) =>
  buildCostElementChanger<TAction>((costElement, action) => ({
    ...costElement,
    inputLevels: {
      ...costElement.inputLevels,
      list: mapIf(
      ...
      list: costBlock.costElements.list.map(costElement =>
        costElement.costElementId === action.costElementId
          ? {
              ...
              : { ...costElement, loadDependencyNeeded: true }
      )
    }
  })
);
...
const setPreviousRegion = (
  ...
  if (oldCostElementId) {
    ...
    if (
      newCostElementMeta.id != oldCostElementMeta.id &&
      newCostElementMeta.regionInput &&
      oldCostElementMeta.regionInput &&
      newCostElementMeta.regionInput.id == oldCostElementMeta.regionInput.id
    ) {
      const oldCostElement = findCostElement(
        costBlock.costElements,
        oldCostElementId
      );
...
const selectCostElement = buildCostBlockChanger<CostElementMetaAction>(
...
const changeSelectionCostElementFilter = buildCostElementChanger<
  CostElementFilterSelectionChangedAction
>((costElement, action) => ({
  ...costElement,
...
const resetCostElementFilter = buildCostElementChanger<CostElementMetaAction>(
  costElement => ({
    ...costElement,
  ...
  (costBlock, action) => ({
    ...
    costElements: {
      ...
      list: mapIf(
        costBlock.costElements.list,
        costElement =>
          costElement.costElementId == costBlock.costElements.selectedItemId,
        costElement => ({
          ...costElement,
          inputLevels: {
            ...costElement.inputLevels,
...
const changeSelectionInputLevelFilter = buildInputLevelFilterChanger<
  InputLevelFilterSelectionChangedAction
>((inputLevel, action) => ({
  ...inputLevel,
...
const resetInputLevelFilter = buildInputLevelFilterChanger(inputLevel => ({
  ...inputLevel,
            ...
            <CostElementState>{
              ...
              region: {
                ...
                list:
                  ...
                  costElementData.regions.sort((region1, region2) =>
                    region1.name.localeCompare(region2.name)
                  ),
                selectedItemId:
...
const loadDependencyFilter = buildCostElementChanger<
  DependencyFilterLoadedAction
>(
  (costElement, { filterItems }) =>
    <CostElementState>{
      ...costElement,
      loadDependencyNeeded: false,
      filter: loadFilter(filterItems)
    }
);

const loadLevelInputFilter = buildInputLevelFilterChanger<
  InputLevelFilterLoadedAction
>((inputLevel, action) => ({
  ...inputLevel,
...
const clearEditItems = buildCostBlockChanger(costblock => ({
  ...costblock,
  edit: {
    ...costblock.edit,
    editedItems: []
...
const checkedFilterItemsSet = (filterItems: CheckItem[]) => {
  const ids =
    filterItems &&
    filterItems.filter(item => item.isChecked).map(item => item.id);

  return new Set<number>(ids);
};

const applyFilters = buildCostBlockChanger(costBlock => {
  const { costElements } = costBlock;

  const selectedCostElement = costElements.list.find(
    item => item.costElementId === costElements.selectedItemId
  );

  const selectInputLevel = selectedCostElement.inputLevels.list.find(
    item => item.inputLevelId === selectedCostElement.inputLevels.selectedItemId
  );
...
const resetErrors = buildCostBlockChanger(costBlock => ({
  ...costBlock,
  edit: {
    ...costBlock.edit,
    saveErrors: []
...
const selectCostBlock: Reducer<CostEditorState, CostBlockAction> = (
  state,
  { applicationId, costBlockId }
) =>
  mapApplication(
    state,
    applicationId,
...
export const costEditorReducer: Reducer<CostEditorState, Action<string>> = (
  state = defaultState(),
  action
) => {
  switch (action.type) {
    ...
    case COST_BLOCK_INPUT_LOAD_DEPENDENCY_FILTER:
      return loadDependencyFilter(state, <DependencyFilterLoadedAction>action);
