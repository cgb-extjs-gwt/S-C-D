...
import {
  getCostBlock,
  findMeta,
  getCostElementByAppMeta,
  getCostElement
} from "../../Common/Helpers/MetaHelper";
import {
  selectCostElement,
  selectInputLevel,
  loadInputLevelFilter,
  editItemsUrlChanged,
  selectRegion,
  applyFilters,
  saveEditItems,
  loadCostElementData,
  loadDependencyFilter
} from "./CostBlockActions";
import {
  buildCostEditorContext,
  findCostElementByState,
  findInputeLevelByState,
  findInputLevel,
...
export const loadDataByCostElementSelection = (
  applicationId: string,
  costBlockId: string,
  costElementId: string
) =>
  asyncAction<CommonState>((dispatch, getState) => {
    const {
      app: { appMetaData }
    } = getState();
    ...
    const costElement = findCostElementByState(
      costEditor,
      applicationId,
      costBlockId,
      costElementId
    ...
    if (
      ...
      (costElement.loadDependencyNeeded && costElement.isDataLoaded)
    ) {
    ...
    if (costElement.inputLevels.selectedItemId == null) {
      dispatch(
        loadDataByInputLevelSelection(
          ...
          costBlockId,
          costElementId,
          costElement.inputLevels.list[0].inputLevelId
        )
      );
    }
  });

export const loadDataByInputLevelSelection = (
  applicationId: string,
  costBlockId: string,
  costElementId: string,
  inputLevelId: string
  ...
  asyncAction<CommonState>((dispatch, getState) => {
    dispatch(
      selectInputLevel(applicationId, costBlockId, costElementId, inputLevelId)
    );

    const state = getState();
    const costEditor = state.pages.costEditor;
    const costBlockMeta = findMeta(
      state.app.appMetaData.costBlocks,
      costBlockId
    );
    ...
    if (inputLevelMeta.hasFilter) {
      const inputLevel = findInputeLevelByState(
        costEditor,
        applicationId,
        costBlockId,
        costElementId,
...
export const loadEditItemsByContext = () =>
  asyncAction<CommonState>((dispatch, getState) => {
    ...
    if (context.costElementId != null && context.inputLevelId != null) {
      const costElementMeta = findMeta(
      ...
      if (!costElementMeta.regionInput || context.regionInputId) {
        ...
        isDispatching =
          costElementMeta.dependency == null || costElementState.filter != null;

        if (isDispatching !== false) {
          const inputLevelMeta = findMeta(
            costElementMeta.inputLevels,
            context.inputLevelId
          );

          if (inputLevelMeta.hasFilter) {
            const inputLevelState = findInputLevel(
              costElementState.inputLevels
            );
...
export const saveEditItemsToServer = (
  applicationId: string,
  costBlockId: string,
  ...
  asyncAction<CommonState>((dispatch, getState) => {
    ...
    handleRequest(
      service
        .saveEditItems(costBlock.edit.editedItems, context, approvalOption)
        .then(qualityGateResult =>
          dispatch(saveEditItems(applicationId, costBlockId, qualityGateResult))
        )
    );
  });

export const selectRegionWithReloading = (
  applicationId: string,
  costBlockId: string,
  regionId: number
) =>
  asyncAction<CommonState>((dispatch, getState) => {
    ...
    const { costElements } = findCostBlockByState(
      costEditor,
      applicationId,
      costBlockId
    );
...
export const applyFiltersWithReloading = (
  applicationId: string,
  costBlockId: string
) =>
  asyncAction<CommonState>(dispatch => {
    dispatch(applyFilters(applicationId, costBlockId));
    dispatch(loadEditItemsByContext());
  });

const loadDependencyFilterFromServer = (
  dispatch: Dispatch,
  state: CommonState
) =>
  new Promise<any>(resolve => {
    ...
    if (costBlock.costElements.selectedItemId) {
      const costElementMeta = getCostElementByAppMeta(
        appMetaData,
        costBlock.costBlockId,
        costBlock.costElements.selectedItemId
      );
