...
{
    public class ReleaseSelectedHwCost
    {
        private const string PROC = "Hardware.SpReleaseCosts";

        private readonly IRepositorySet _repo;

        public ReleaseSelectedHwCost(IRepositorySet repo)
        {
            _repo = repo;
        }

        public async Task ExecuteAsync(long userId, HwFilterDto filter, HwCostDto[] items)
        {
