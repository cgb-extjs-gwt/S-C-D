...
namespace Gdc.Scd.BusinessLogicLayer.Import
{
    public class PorHwFspCodeTranslationService : ImportService<HwFspCodeTranslation>, IHwFspCodeTranslationService
    {
        ...
        {
            var record = matrix.FirstOrDefault(m => m.AvailabilityId == code.AvailabilityId &&
                                                    m.DurationId == code.DurationId &&
                                                    m.ReactionTimeId == code.ReactionTimeId &&
                                                    m.ReactionTypeId == code.ReactionTypeId &&
                                                    m.ServiceLocationId == code.ServiceLocationId &&
                                                    m.WgId == code.WgId &&
                                                    m.CountryId == code.CountryId);
            return record?.Id;
        }
